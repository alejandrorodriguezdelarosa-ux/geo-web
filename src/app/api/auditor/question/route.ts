...
export async function POST(req: NextRequest) {
  ...
  const { mode, text, url, title, niche, prompt } = parsed.data
  ...
  const requestBody = mode === "text"
    ? { text, prompt, niche: niche || undefined, title: title || "" }
    : { url, prompt, niche: niche || undefined }
  let res: Response
  try {
    res = await fetch(`${geoApiBase}/api/measure-question`, {
      method: "POST",
